...
#	  echo "neutral" >$(tty)
  ...
  # excl & zero maps
  #
  bundle_frag_maps "$fragments" "allprobes_drude.map"

  cat allprobes_drude.map | awk '{if(NR<=6) {print $0} else {s=0 ;if($1<0.9){s=1000}else{s=0} ; printf "%10.3f\n",s}}' > ${mapdir}/${pdb}.excl.${startrun}-${endrun}.map

  #rm -f allprobes_drude.map
